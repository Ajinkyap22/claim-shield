...
export function getScoreConfig(score: number) {
  if (score < 30) return { color: "#16a34a", bg: "#dcfce7", label: "Low Risk", textColor: "#15803d" };
  if (score < 60) return { color: "#ca8a04", bg: "#fef9c3", label: "Moderate Risk", textColor: "#a16207" };
  if (score < 80) return { color: "#ea580c", bg: "#ffedd5", label: "Elevated Risk", textColor: "#c2410c" };
  return { color: "#dc2626", bg: "#fee2e2", label: "High Risk", textColor: "#b91c1c" };
}
...
export function ScoreGauge({ score, animated = true }: ScoreGaugeProps) {
  ...
  const scoreArcPathStr = displayScore > 0
    ? arcPath(CX, CY, R, 180, Math.max(scoreAngle, 0.5))
    : "";
    ...
    <div className="flex flex-col items-center w-full">
      ...
      >
        ...
        {ZONES.map((zone, i) => (
          <path
            key={i}
            d={arcPath(CX, CY, R, scoreToAngle(zone.from), scoreToAngle(zone.to))}
            fill="none"
            stroke={zone.color}
        ...
        <line
          ...
          filter="url(#gaugeShadow)"
        />
        <circle cx={CX} cy={CY} r={9} fill={config.color} filter="url(#gaugeShadow)" />
        ...
        <text
          ...
          y={CY + 38}
          textAnchor="middle"
          style={{ fontSize: "44px", fontWeight: 800, fill: config.color, letterSpacing: "-2px" }}
        >
          {displayScore}
        </text>
        <text
          ...
          y={CY + 54}
          textAnchor="middle"
          style={{ fontSize: "11px", fill: "#94a3b8", fontWeight: 500, letterSpacing: "0.06em" }}
        >
          DENIAL RISK / 100
        </text>
      </svg>
      ...
      <div
        ...
        style={{ backgroundColor: config.bg, color: config.textColor, fontSize: "0.85rem", fontWeight: 700 }}
      >
        {config.label}
      </div>
      ...
      <div className="flex gap-3 mt-3 flex-wrap justify-center">
        {[
          ...
          <div key={z.label} className="flex items-center gap-1">
            ...
            <span style={{ fontSize: "0.68rem", color: "#64748b" }}>{z.label}</span>
          </div>
        ))}
      </div>
    </div>
